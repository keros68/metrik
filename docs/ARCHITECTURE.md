...
- **OpenCode Go**: hangs on the OpenCode card, which also carries OpenCode's local usage. One live GET to the official endpoint using the `opencode-go` key from the local OpenCode `auth.json` (or `OPENCODE_GO_API_KEY`); `percent` is the *used* percentage and is converted to remaining before storage. The response shape comes from a reference implementation and has **not been verified against a real account**.
