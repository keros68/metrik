...
- **Merge the release PR with a merge commit, never squash.** Squashing rewrites the release commit under a new SHA, so the tag you already pushed no longer sits in `main`'s history — and `version-guard` checks exactly that (`git merge-base --is-ancestor "$tag^{commit}" HEAD`), so every subsequent PR fails with "已被占用且不在本次提交的历史内" until it is repaired. This is the one case where retargeting a tag is correct rather than burning the number: the squashed commit has an identical tree, so the published artifacts still match the code. Verify the trees agree (`git rev-parse v0.10.0^{tree}` vs `HEAD^{tree}`), then `git tag -f vX.Y.Z HEAD && git push --force origin vX.Y.Z`. Do not retag when the content differs — that is the case the burn rule exists for.
