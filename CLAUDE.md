...
- **后端**（`src-tauri/src/`）：UI 调用异步命令 `usage_snapshot(period)`（另有 `rebuild_local_ledger`、`sync_settings`、`configure_sync`）。`engine.rs` 是扫描/对账核心（单一扫描锁 + `spawn_blocking`）；`adapters/`（codex.rs、claude.rs、opencode.rs）实现 `AgentAdapter` trait；`storage.rs` + `schema.rs` 管 SQLite 账本（WAL、迁移、恢复）；`sync.rs` 实现共享文件夹多设备同步（导出/导入只含统计字段的 JSON）；`app_server.rs` 取 Codex 官方配额；`domain.rs` 是共享类型，`AGENT_IDS` 是启用 Agent 的注册表（新 Agent 需同时更新前端 `AGENT_META`）。
...
## 双机并行开发（Windows / macOS 各一台，改动前必读）

完整规则见 `AGENTS.md` 的 "Durable cross-platform development workflow"，那里是唯一出处；不要在仓库外维护副本。要点：

- 每个改动先归类：`shared`（adapter、配额解析、storage、sync、设置契约、统计——写一次，两个平台都要验）、`macOS shell`、`Windows shell`。两个 shell 的窗口形态/材质/定位/托盘交互互不抄袭，各自只在本平台开发与验收。
- 平台分支只认 Rust `cfg(target_os)` 与 Tauri 编译期平台信号；**不得**用 WebView UA 判断当平台开关。
- **版本号是两边共享的全局资源**：一个 app、一个号、一个 tag、一个含双平台产物的 Release，所以发版串行、不能并行。
- 发版协议（两边一致）：先 fetch + rebase → 版本号只在发版提交里改（特性分支绝不带版本改动）→ `git push origin main vX.Y.Z` 代码与 tag 一次推送（tag 才是占号的锁）→ 双平台 CI 绿 + 检查 draft 里两平台产物齐全再发布。
- tag 一旦存在即视为该号已烧掉：不复用、不重打，rebase 后取下一个号。CI 的 `version-guard` 会挡住五个版本文件不一致、以及占用他人已 tag 的版本号。
