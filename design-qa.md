...
## 2026-07-15（十）macOS 菜单栏形态真机验收通过

用户在 Apple Silicon 实机上确认，四条核心交互全部符合预期：

1. 面板贴着菜单栏图标正下方弹出（位置正确）。
2. 面板弹出时不抢当前窗口焦点（NSPanel 的核心目的：瞄一眼用量不打断打字）。
3. 点击别处自动收起。
4. 「完整视图」是带原生红绿灯的独立窗口，关掉后退回纯菜单栏。

我预判可能要调的两点——菜单栏 template 图标的线条粗细、面板 tint 偏透——用户实机看下来均可接受，未做改动。

**仍未验收：** macOS 的开机启动（LaunchAgent 路径）没有逐项核过；Kimi 与 Antigravity 适配依旧无真机（本机未安装这两个 Agent）。

README 的验收边界与平台徽章已按实际确认范围更新：确认到哪儿写到哪儿，没试的继续如实标着未验。
